...
function timeSpanToString(startDate, endDate) {
  const rez = endDate.getTime() - startDate.getTime();
...
function angleBetweenClockHands(date) {
  const hoursInMillisec = Math.floor(date / (60 * 60 * 1000));
  const minutes = (date - (hoursInMillisec * 60 * 60 * 1000)) / (60 * 1000);
  const hours = (hoursInMillisec % 24) % 12;
