...
function isCreditCardNumber(ccn) {
  ...
  if (sum % 10 === 0) return sum % 10 === +str[0];
  return 10 - (sum % 10) === +str[0];
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (Object.keys(openBrackets).includes(arr[i])) stack.push(arr[i]);
    if (stack.length === 0) return false;
    if (closeBrackets.includes(arr[i])) {
      const currentEl = stack.pop();
      if (arr[i] !== openBrackets[currentEl]) return false;
    }
  }
